...
class BFButton(object):
    ...
    def init_font(self):
        ...
        white = 100, 100, 100
        self.textImage = font.render(self.text, True, white)
    ...
    def update(self, event):
        ...
        if self.x < x < self.x + self.width and self.y < y < self.y + self.height:
            ...
            if event.type == MOUSEBUTTONDOWN:
                pressed_array = pygame.mouse.get_pressed(num_buttons=3)
                if pressed_array[0]:
                    self.in_click = True
    ...
    def draw(self):
        ...
        if self.is_hover:
            ...
            pygame.draw.rect(self.surface, (100, 100, 100), (0, 0, self.width - 1, self.height - 1),
                             1)
            layers = 5
            r_step = (210 - 170) / layers
            g_step = (225 - 205) / layers
            for i in range(layers):
                pygame.draw.rect(self.surface, (int(170 + r_step * i), int(205 + g_step * i), 255),
                                 (i, i, self.width - 2 - i * 2, self.height - 2 - i * 2), 1)
        else:
            self.surface.fill(self.bg_color)
            pygame.draw.rect(self.surface, (0, 0, 0), (0, 0, self.width, self.height), 1)
            pygame.draw.rect(self.surface, (100, 100, 100), (0, 0, self.width - 1, self.height - 1),
                             1)
            pygame.draw.rect(self.surface, self.bg_color, (0, 0, self.width - 2, self.height - 2),
                             1)
